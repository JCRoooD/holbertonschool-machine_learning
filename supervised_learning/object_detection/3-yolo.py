...
class Yolo:
    ...
    def filter_boxes(self, boxes, box_confidences, box_class_probs):
        ...
        """
        # List to hold all the filtered boxes
        filtered_boxes = []
        # List to hold all the box classes
        box_classes = []
        # List to hold all the box scores
        box_scores = []
        ...
        for box_confidence, box_class_prob, box in zip(
            ...
            # filter out boxes based on the box score
            ...
            # Append the filtered boxes to the list
    ...
    def non_max_suppression(self, filtered_boxes, box_classes, box_scores):
        ...
        predicted_box_scores):
            ...
            predicted_box_scores: a numpy.ndarray of shape (?) containing the
                                  ...
                                  class and box score, respectively
        """
        # List to hold the box predictions
        box_predictions = []
        # List to hold the predicted box classes
        predicted_box_classes = []
        # List to hold the predicted box scores
        predicted_box_scores = []

        # Iterate over the unique classes
        for unique_class in np.unique(box_classes):
            # find indices of all boxes 
            indices = np.where(box_classes == unique_class)[0]

            # Extract the boxes for the current class
            ...
            # Sort the boxes by the scores descending_order
            order = np.argsort(box_scores_subset)[::-1]


            # initialize list to hold the final predictions
            final_predictions = []

            while order.size > 0:
                # take the first box in the sorted list
                final_predictions.append(order[0])

                # calculate the intersection over union of the first box
                ...
                yy2 = np.minimum(x2[order[0]], y2[order[1:]])

                intersection = np.maximum(0, xx2 - xx1 + 1) * np.maximum(
                    0, yy2 - yy1 + 1)
                union = box_areas[order[0]] + box_areas[
                    order[1:]] - intersection
                IOU = intersection / union

                # keep only the boxes with IOU less than the threshold
                update_order = np.where(IOU <= self.nms_t)[0]
                order = order[update_order + 1]

            final_predictions = np.array(final_predictions)
            box_predictions.append(filtered_boxes_subset[final_predictions])
            predicted_box_classes.append(box_classes_subset[final_predictions])
            predicted_box_scores.append(box_scores_subset[final_predictions])
