...
def baum_welch(Observations, Transition, Emission, Initial, iterations=1000):
    ...
    for iteration in range(iterations):
        ...
        for t in range(observations):
            for i in range(hidden_states):
                Fit = F[i, t]  # Forward probability at time t for state i
                Bit = B[i, t]  # Backward probability at time t for state i
                NUM[i, t] = Fit * Bit  # Numerator for gamma

        # Compute the denominator for gamma
        DEN = np.sum(NUM, axis=0)
        G = NUM / DEN  # Gamma values

        # Update the Transition matrix using the sum of X and G
        Transition = np.sum(
            X, axis=2) / np.sum(
                G[:, :observations - 1], axis=1)[..., np.newaxis]

        # Update the Emission matrix using the sum of G and Observations
        DEN = np.sum(G, axis=1)
